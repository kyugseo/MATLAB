...
class mpfit:
	...
	blas_enorm32, = scipyblas.get_blas_funcs(['nrm2'],numpy.array([0],dtype=numpy.float32))
	blas_enorm64, = scipyblas.get_blas_funcs(['nrm2'],numpy.array([0],dtype=numpy.float64))
  ...
  Keywords:
   ...
   Outputs:
	 ...
	 .perror
		...
		*If* you can assume that the true reduced chi-squared value is unity --
		...
		if parinfo is not None:
			if type(parinfo) != list: #types.ListType: #JW modified 28 nov. 2018
				self.errmsg = 'ERROR: PARINFO must be a list of dictionaries.'
				return
			else:
				if type(parinfo[0]) != dict:#types.DictionaryType:#JW modified 28 nov. 2018
					self.errmsg = 'ERROR: PARINFO must be a list of dictionaries.'
					return
	...
	def parinfo(self, parinfo=None, key='a', default=None, n=0):
		...
		values = []
		for i in range(n):
			if (parinfo is not None) and (key in parinfo[i].keys()): #JW modified 28 Nov. 2018
		...
		test = default
		if type(default) == list:#types.ListType:#JW modified 28 Nov. 2018
			test=default[0]
		if isinstance(test, int):#types.IntType):#JW modified 28 Nov. 2018
			values = numpy.asarray(values, int)
		elif isinstance(test, float):#types.FloatType):#JW modified 28 Nov. 2018
