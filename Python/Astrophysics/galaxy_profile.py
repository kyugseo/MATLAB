...
pixscale = 0.8142039473684209# Use the plate scale (arsec/pixel) that you found in Assignment 2 (note that is for binned pixels of 2x2)
...
'''
    ...
    into new columns of the objects Table.  We want to save:
        flux/texp into the column "flux"
        fluxerr/texp into the column "fluxerr"
...
def calibrateRefStar(objects,refmag): # refmag is the true magnitude of the reference star
    ...
    flux_conversion_factor = 1/10**(mag_offset/-2.5)
    print ('flux_conversion_factor :',flux_conversion_factor)
...
def galaxyProfile(image_data_nosky,noise,objects,seg,seg_id,band):
    ...
    # The column 'theta' is the CCW angle between the ellipse and the horizontal.  

    # The Kron radii which we computed above are in units of "a" and "b".  It has been shown that 2.5 times 
    ...
    flux_ellipses, fluxerr, flag = sep.sum_ellipse(image_data_nosky,xgal,ygal,a,b,theta,r=rfractions,subpix=5,err=noise,segmap=seg,seg_id=sid)
...
def plotSBProfile(tab,flux_conversion_factor,texp,band,ax):
    ...
    mu = 2.5*np.log10(tab['area_annuli']*pixscale**2) - 2.5*np.log10(correctedFlux/texp)  # surface brightness in mag/arcsec^2
    mu_err = np.abs((-2.5/correctedFlux)/np.log(10)*correctedFluxErr) # The errors using standard error propagation.
...
# Notice that I have put in the extinction coefficients and magnitudes of the reference stars here.
